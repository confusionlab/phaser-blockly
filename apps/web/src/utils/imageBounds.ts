...
export function calculateAlphaBoundsPairFromCanvasRegion(
  canvas: HTMLCanvasElement,
  region: CostumeBounds,
  boundsAlphaThreshold: number = 10,
  cropAlphaThreshold: number = 0,
): AlphaBoundsPair {
  const normalizedX = Math.max(0, Math.floor(region.x));
  const normalizedY = Math.max(0, Math.floor(region.y));
  const normalizedRight = Math.min(canvas.width, Math.ceil(region.x + region.width));
  const normalizedBottom = Math.min(canvas.height, Math.ceil(region.y + region.height));
